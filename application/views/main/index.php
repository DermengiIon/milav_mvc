<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<title>Milav - Improving your performance</title>
	<meta name="description" content="Servicii web">
	<meta http-equiv="X-UA-Compatible" content="IE=edge">

	<meta name="author" content="Dermengi Ion">
	<meta name="keywords" content="servicii, web, design, milav, IT">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">

	<link rel="shortcut icon" type="image/png" href="../../public/img/favicon.ico.png"/>
	<link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.0.13/css/all.css" integrity="sha384-DNOHZ68U8hZfKXOrtjWvjxusGo9WQnrNx2sqG0tfsghAvtVlRW3tvkXWZh58N9jp" crossorigin="anonymous"> 
	<link rel="stylesheet" href="../../public/css/custom/main.min.css">

</head>
<body class="home-page">
	<header class="header-page">
		<nav>
			<div class="logo">
				<a href="iondermengi.milav.eu"><img src="../../public/img/logo.png" alt="Servicii web Milav IT" /></a>
			</div>
			<div class="burger-container">
				<div id="burger">
					<div class="bar topBar"></div>
					<div class="bar btmBar"></div>
				</div>
			</div>
			<ul class="nav">
				<li class="nav-item"><a href="">Home</a></li>
				<li class="nav-item"><a href="#services">Services</a></li>
				<li class="nav-item"><a href="#portfolio">Portfolio</a></li>
				<li class="nav-item"><a href="#testimonials">Testimonials</a></li>
				<li class="nav-item"><a href="#about">About us</a></li>
				<li class="nav-item"><a href="#jobs">Jobs</a></li>
				<li class="nav-item"><a href="#contact">Contact</a></li>
			</ul>
		</nav>
		<div class="slider">
			<img src="../../public/img/slide1.jpg" alt="">
			<img src="../../public/img/slide2.jpg" alt="">
			<img src="../../public/img/slide3.jpg" alt="">
			<img src="../../public/img/slide4.jpg" alt="">
			<div class="slider-hero"></div>
		</div>
	</div>
	<canvas class="bg-canvas"></canvas>
	<div class="wrapper">
		<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
		viewBox="0 0 541 415" style="enable-background:new 0 0 541 415;" xml:space="preserve">
		<style type="text/css">
		.st0{fill:#fff;stroke:#FFFFFF;stroke-width:3;stroke-miterlimit:10;}
	</style>
	<g>
		<path class="st0" d="M184.5,154.6v100H156v-71.5l-18.5,71.5H118l-19.2-71.5v71.5h-23v-100h40.1l14.5,53.1l13.8-53.1H184.5z"/>
		<path class="st0" d="M232.6,154.6v100h-30.7v-100H232.6z"/>
		<path class="st0" d="M320.2,230.1v24.5h-70.1v-100h30.7v75.4H320.2z"/>
		<path class="st0" d="M386,254.6l-5.6-19.7h-26.8l-6,19.7h-24.9l30.9-100h32.4l31.4,100H386z M358.8,213.1h16.2l-8.1-29L358.8,213.1	z"/>
		<path class="st0" d="M507,154.6l-31,100h-31.3l-32.4-100h32.3l15.8,54.5c1.1,3.2,2,6.4,2.9,9.5l18.5-64H507z"/>
	</g>
</svg>
</div>
<div class="typing">
	<h2 id="writer"></h2>
</div>
<div class="social-media-icons"><a title="We are here ^_^" class="facebook" href="https://www.facebook.com/milav.eu" target="_blank"><img src="../../public/img/facebook.png" alt="milav facebook"><div>We are here ^_^</div></a><a title="LinkedIn" class="linkedin" href="https://www.linkedin.com/company/4866175/" target="_blank"><img src="../../public/img/linkedin.png" alt="milav linkedin"></a></div>				
</header>
<div class="contact-buttons">
	<ul>
		<li><a href="tel:0040747191825">
			<svg version="1.1" id="contacts" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
			viewBox="0 0 36.539 36.539" style="enable-background:new 0 0 36.539 36.539;width: 24px;" xml:space="preserve">
			<g>
				<path d="M4.09,15.786c-1.283,0-2.246-0.309-2.926-0.935c-1.238-1.139-1.19-2.986-1.148-4.615c0.007-0.269,0.014-0.53,0.014-0.779
				C0.028,7.718,1.933,5.819,5.122,4.376c3.509-1.587,8.177-2.461,13.146-2.461c4.971,0,9.64,0.874,13.148,2.461
				c3.19,1.442,5.094,3.342,5.093,5.08c0,0.251,0.007,0.514,0.014,0.782c0.041,1.629,0.088,3.475-1.148,4.613
				c-0.854,0.785-2.155,1.073-3.982,0.873c-5.39-0.586-5.373-2.732-5.354-5.219l0.002-0.442c0-0.41-0.561-1.748-7.772-1.748
				c-2.354,0.001-4.218,0.143-5.54,0.423c-0.833,0.177-2.23,0.583-2.23,1.324L10.5,10.51c0.018,2.485,0.033,4.631-5.354,5.215
				C4.773,15.766,4.421,15.786,4.09,15.786z M18.269,2.915c-4.83,0-9.353,0.843-12.734,2.372c-2.737,1.237-4.505,2.874-4.505,4.17
				c0,0.258-0.007,0.528-0.015,0.805c-0.036,1.417-0.077,3.022,0.826,3.854c0.62,0.57,1.699,0.777,3.196,0.614
				c4.49-0.486,4.479-1.89,4.463-4.213l-0.002-0.455C9.497,8.96,10.515,8.185,12.522,7.76c1.39-0.294,3.323-0.443,5.747-0.444l0,0
				c5.985,0,8.772,0.873,8.772,2.748l-0.002,0.45c-0.019,2.325-0.029,3.729,4.461,4.217c1.507,0.164,2.577-0.044,3.197-0.615
				c0.903-0.83,0.862-2.436,0.826-3.852c-0.007-0.277-0.014-0.549-0.014-0.808c0.001-1.295-1.768-2.932-4.505-4.169
				C27.623,3.758,23.1,2.915,18.269,2.915z"/>
				<path d="M11.24,21.956H6.573c-0.276,0-0.5-0.224-0.5-0.5v-3c0-0.276,0.224-0.5,0.5-0.5h4.667c0.276,0,0.5,0.224,0.5,0.5v3
				C11.74,21.733,11.517,21.956,11.24,21.956z M7.073,20.956h3.667v-2H7.073V20.956z"/>
				<path d="M20.603,21.956h-4.667c-0.276,0-0.5-0.224-0.5-0.5v-3c0-0.276,0.224-0.5,0.5-0.5h4.667c0.276,0,0.5,0.224,0.5,0.5v3
				C21.103,21.733,20.879,21.956,20.603,21.956z M16.436,20.956h3.667v-2h-3.667V20.956z"/>
				<path d="M29.964,21.956h-4.666c-0.276,0-0.5-0.224-0.5-0.5v-3c0-0.276,0.224-0.5,0.5-0.5h4.666c0.276,0,0.5,0.224,0.5,0.5v3
				C30.464,21.733,30.24,21.956,29.964,21.956z M25.798,20.956h3.666v-2h-3.666V20.956z"/>
				<path d="M11.24,28.289H6.573c-0.276,0-0.5-0.224-0.5-0.5v-3c0-0.276,0.224-0.5,0.5-0.5h4.667c0.276,0,0.5,0.224,0.5,0.5v3
				C11.74,28.066,11.517,28.289,11.24,28.289z M7.073,27.289h3.667v-2H7.073V27.289z"/>
				<path d="M20.603,28.289h-4.667c-0.276,0-0.5-0.224-0.5-0.5v-3c0-0.276,0.224-0.5,0.5-0.5h4.667c0.276,0,0.5,0.224,0.5,0.5v3
				C21.103,28.066,20.879,28.289,20.603,28.289z M16.436,27.289h3.667v-2h-3.667V27.289z"/>
				<path d="M29.964,28.289h-4.666c-0.276,0-0.5-0.224-0.5-0.5v-3c0-0.276,0.224-0.5,0.5-0.5h4.666c0.276,0,0.5,0.224,0.5,0.5v3
				C30.464,28.066,30.24,28.289,29.964,28.289z M25.798,27.289h3.666v-2h-3.666V27.289z"/>
				<path d="M11.24,34.623H6.573c-0.276,0-0.5-0.224-0.5-0.5v-3c0-0.276,0.224-0.5,0.5-0.5h4.667c0.276,0,0.5,0.224,0.5,0.5v3
				C11.74,34.4,11.517,34.623,11.24,34.623z M7.073,33.623h3.667v-2H7.073V33.623z"/>
				<path d="M20.603,34.623h-4.667c-0.276,0-0.5-0.224-0.5-0.5v-3c0-0.276,0.224-0.5,0.5-0.5h4.667c0.276,0,0.5,0.224,0.5,0.5v3
				C21.103,34.4,20.879,34.623,20.603,34.623z M16.436,33.623h3.667v-2h-3.667V33.623z"/>
				<path d="M29.964,34.623h-4.666c-0.276,0-0.5-0.224-0.5-0.5v-3c0-0.276,0.224-0.5,0.5-0.5h4.666c0.276,0,0.5,0.224,0.5,0.5v3
				C30.464,34.4,30.24,34.623,29.964,34.623z M25.798,33.623h3.666v-2h-3.666V33.623z"/>
			</g>
		</svg></a></li>
		<li><a href="mailto:csatlos.sandor@milav.eu">
			<svg version="1.1" id="contacts" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
			viewBox="0 0 31.012 31.012" style="enable-background:new 0 0 31.012 31.012; width: 24px;" xml:space="preserve">
			<g>
				<path d="M25.109,21.51c-0.123,0-0.246-0.045-0.342-0.136l-5.754-5.398c-0.201-0.188-0.211-0.505-0.022-0.706
				c0.189-0.203,0.504-0.212,0.707-0.022l5.754,5.398c0.201,0.188,0.211,0.505,0.022,0.706C25.375,21.457,25.243,21.51,25.109,21.51z
				"/>
				<path d="M5.902,21.51c-0.133,0-0.266-0.053-0.365-0.158c-0.189-0.201-0.179-0.518,0.022-0.706l5.756-5.398
				c0.202-0.188,0.519-0.18,0.707,0.022c0.189,0.201,0.179,0.518-0.022,0.706l-5.756,5.398C6.148,21.465,6.025,21.51,5.902,21.51z"/>
			</g>
			<path d="M28.512,26.529H2.5c-1.378,0-2.5-1.121-2.5-2.5V6.982c0-1.379,1.122-2.5,2.5-2.5h26.012c1.378,0,2.5,1.121,2.5,2.5v17.047
			C31.012,25.408,29.89,26.529,28.512,26.529z M2.5,5.482c-0.827,0-1.5,0.673-1.5,1.5v17.047c0,0.827,0.673,1.5,1.5,1.5h26.012
			c0.827,0,1.5-0.673,1.5-1.5V6.982c0-0.827-0.673-1.5-1.5-1.5H2.5z"/>
			<path d="M15.506,18.018c-0.665,0-1.33-0.221-1.836-0.662L0.83,6.155C0.622,5.974,0.6,5.658,0.781,5.449
			c0.183-0.208,0.498-0.227,0.706-0.048l12.84,11.2c0.639,0.557,1.719,0.557,2.357,0L29.508,5.419
			c0.207-0.181,0.522-0.161,0.706,0.048c0.181,0.209,0.16,0.524-0.048,0.706L17.342,17.355
			C16.835,17.797,16.171,18.018,15.506,18.018z"/>
		</g>
	</svg>
</a></li>
</ul>
</div>
<section id="services" class="services">
	<div class="container">
		<div class="row">
			<div class="col-md-10 offset-md-1">
				<div class="col-sm-6 col-xs-12 services-item" >
					<div class="services-item-img"><img src="../../public/img/yii2.png" alt="milav yii2"></div>
					<p>Yii means “simple and evolutionary” in Chinese. But for us it means one of the best web application frameworks that we know... <span class="services-more">more</span></p>
					<div class="more">
					<p>	Designed to work well with third-party code, it’s highly extendable, pretty fast to code and it utilizes PHP’s most advanced features.</p>
					<p> Do you need to develop real-time applications? Maybe with Ajax support included?</p>
			        <p>	Than Yii2 is a good solution for you.</p>
			        </div>
				</div>
				<div class="col-sm-6 col-xs-12 offset-md-6 services-item" >
					<div class="services-item-img"><img src="../../public/img/html5.png" alt="milav html5"></div>
					<p>The Internet is ubiquitous and we can’t work without it for a long time. The global network connects all devices that can render a website...<span class="services-more">more</span></p>
					<div class="more">
						<p> HTML5 and CSS3 and Javascript are essential ingredients of your modern, responsive website and we have the promise of truly open on any modern day device you can imagine</p>
                       <p>Want some beautiful and clean web design to work on desktop but also on mobile? HTML5 and CSS3 is the key for correctly embed text content, images and videos. Want something dynamic on your website? Animating the images, or maybe some smart widgets? Javascript is the best solutions for you.</p> 
					</div>
				</div>
				<div class="col-sm-6 col-xs-12 services-item" >
					<div class="services-item-img"><img src="../../public/img/php.png" alt="milav php"></div>
					<p>Did you know that PHP is one of the biggest programing languages out there? And it has a lot of advantages, like fast load time, good for complex calculations, good connectivity with databases and endless possibilities for your ideas...<span class="services-more">more</span></p>
					<div class="more">
						<p>That’s why we use it to provide web applications for our clients.</p>
                        <p>This is how we work: product analysis, design, implementation and maintenance.</p>
                        <p>What is your idea?</p>
					</div>
				</div>
				<div class="col-sm-6 col-xs-12 offset-md-6 services-item" >
					<div class="services-item-img"><img src="../../public/img/angular.png" alt="milav angular"></div>
					<p>jQuery was not the best solution for you? We can give better solutions for your project with Angular...<span class="services-more">more</span></p>
					<div class="more">
						<p>Or just combining them.</p>
                        <p>Starting with basic DOM things like color changes on mouse hover, hiding-showing elements on click, up too advanced data-manipulation, we can create powerful web apps, front-end elements that your end-users interact with.</p>
                        <p>Let us analyze your needs and give you the best solution.</p>
					</div>
				</div>
				<div class="col-sm-6 col-xs-12 services-item" >
					<div class="services-item-img"><img src="../../public/img/wordpress.png" alt="milav wordpress"></div>
					<p>Almost 30% of the websites use Wordpress. That’s like 70 million websites...<span class="services-more">more</span></p>
					<div class="more">
						<p>So it’s no surprise that it has 45.000 plugins for all possible features, several thousand themes to choose from, it’s SEO friendly and can handle your blog, news site, webshop and portfolio in the same time. And it’s easy to upload your content after it’s built . And it’s free. And we really know almost every line of code from it. By heart. Really. Choose Wordpress, choose us.</p>
					</div>
				</div>
				<div class="col-sm-6 col-xs-12 offset-md-6 services-item" >
					<div class="services-item-img"><img src="../../public/img/drupal.png" alt="milav drupal"></div>
					<p>The White House uses Drupal. So does NBA.com and The Economist...<span class="services-more">more</span></p>
					<div class="more">
					<p>Why is Drupal the hot shot of CMS’?</p>
                    <p>Because it has good Security, lots of Plugins and High Scalability for many content types like videos, polls, user management, text, blogs, podcasts etc. for your digital experience.</p>
                    <p>So if you own a Drupal based portal or you plan to build one, you are in the right place.</p>
                   </div>
				</div>
			</div>
		</div>
	</div>
</section>
<section class="hosting">
	<div class="sections-names">
		<label>HOSTING WEB SSD</label>
	</div>
	<div class="hosting-table">
		<ul class="ul-tables">
			<li>
				<table>
					<tr><th class="th-off"></th><th><span class="host-name">BASIC<br>Package</span><br><span class="price">0<sup>99&euro;</sup><sub>/ month</sub></span></th></tr>
					<?php 
                      foreach ($vars['basic'] as $val):
					?>
					<tr><th>SSD Storage</th><td><?php echo $val->ssd_storage;?></td></tr>
					<tr><th>Monthly traffic</th><td><?php echo $val->monthly_traffic;?></td></tr>
					<tr><th>Hosted domains</th><td><?php echo $val->hosted_domains;?></td></tr>
					<tr><th>Mail boxes</th><td><?php echo $val->mail_boxes;?></td></tr>
					<tr><th>Database</th><td><?php echo $val->nr_database;?></td></tr>
					<tr><th>FTP Account</th><td><?php echo $val->ftp_account;?></td></tr>
					<tr><th>Parced Domains</th><td><?php echo $val->parced_domains;?></td></tr>
					<tr><th>Subdomains</th><td><?php echo $val->subdomains;endforeach;?></td></tr>
				</table>
			</li>
			<li>
				<table>
					<tr><th class="th-off th-hide"></th><th><span class="host-name">START<br>Package</span><br><span class="price">2<sup>99&euro;</sup><sub>/ month</sub></span></th></tr>
					<?php 
                      foreach ($vars['start'] as $val):
					?>
					<tr><th class="th-hide">SSD Storage</th><td><?php echo $val->ssd_storage;?></td></tr>
					<tr><th class="th-hide">Monthly traffic</th><td><?php echo $val->monthly_traffic;?></td></tr>
					<tr><th class="th-hide">Hosted domains</th><td><?php echo $val->hosted_domains;?></td></tr>
					<tr><th class="th-hide">Mail boxes</th><td><?php echo $val->mail_boxes;?></td></tr>
					<tr><th class="th-hide">Database</th><td><?php echo $val->nr_database;?></td></tr>
					<tr><th class="th-hide">FTP Account</th><td><?php echo $val->ftp_account;?></td></tr>
					<tr><th class="th-hide">Parced Domains</th><td><?php echo $val->parced_domains;?></td></tr>
					<tr><th class="th-hide">Subdomains</th><td><?php echo $val->subdomains;endforeach;?></td></tr>
				</table>
			</li>
			<li>
				<table>
					<tr><th class="th-off th-hide"></th><th><span class="host-name">BUSINESS<br>Package</span><br><span class="price">3<sup>99&euro;</sup><sub>/ month</sub></span></th></tr>
					<?php 
                      foreach ($vars['business'] as $val):
					?>
					<tr><th class="th-hide">SSD Storage</th><td><?php echo $val->ssd_storage;?></td></tr>
					<tr><th class="th-hide">Monthly traffic</th><td><?php echo $val->monthly_traffic;?></td></tr>
					<tr><th class="th-hide">Hosted domains</th><td><?php echo $val->hosted_domains;?></td></tr>
					<tr><th class="th-hide">Mail boxes</th><td><?php echo $val->mail_boxes;?></td></tr>
					<tr><th class="th-hide">Database</th><td><?php echo $val->nr_database;?></td></tr>
					<tr><th class="th-hide">FTP Account</th><td><?php echo $val->ftp_account;?></td></tr>
					<tr><th class="th-hide">Parced Domains</th><td><?php echo $val->parced_domains;?></td></tr>
					<tr><th class="th-hide">Subdomains</th><td><?php echo $val->subdomains;endforeach;?></td></tr>
				</table>
			</li>
			<li>
				<table>
					<tr><th class="th-off th-hide"></th><th><span class="host-name">PROFESIONAL<br>Package</span><br><span class="price">5<sup>99&euro;</sup><sub>/ month</sub></span></th></tr>
					<?php 
                      foreach ($vars['profesional'] as $val):
					?>
					<tr><th class="th-hide">SSD Storage</th><td><?php echo $val->ssd_storage;?></td></tr>
					<tr><th class="th-hide">Monthly traffic</th><td><?php echo $val->monthly_traffic;?></td></tr>
					<tr><th class="th-hide">Hosted domains</th><td><?php echo $val->hosted_domains;?></td></tr>
					<tr><th class="th-hide">Mail boxes</th><td><?php echo $val->mail_boxes;?></td></tr>
					<tr><th class="th-hide">Database</th><td><?php echo $val->nr_database;?></td></tr>
					<tr><th class="th-hide">FTP Account</th><td><?php echo $val->ftp_account;?></td></tr>
					<tr><th class="th-hide">Parced Domains</th><td><?php echo $val->parced_domains;?></td></tr>
					<tr><th class="th-hide">Subdomains</th><td><?php echo $val->subdomains;endforeach;?></td></tr>
				</table>
			</li>
			<li>
				<table>
					<tr><th class="th-off th-hide"></th><th><span class="host-name">CORPORATE<br>Package</span><br><span class="price">9<sup>99&euro;</sup><sub>/ month</sub></span></th></tr>
					<?php 
                      foreach ($vars['corporate'] as $val):
					?>
					<tr><th class="th-hide">SSD Storage</th><td><?php echo $val->ssd_storage;?></td></tr>
					<tr><th class="th-hide">Monthly traffic</th><td><?php echo $val->monthly_traffic;?></td></tr>
					<tr><th class="th-hide">Hosted domains</th><td><?php echo $val->hosted_domains;?></td></tr>
					<tr><th class="th-hide">Mail boxes</th><td><?php echo $val->mail_boxes;?></td></tr>
					<tr><th class="th-hide">Database</th><td><?php echo $val->nr_database;?></td></tr>
					<tr><th class="th-hide">FTP Account</th><td><?php echo $val->ftp_account;?></td></tr>
					<tr><th class="th-hide">Parced Domains</th><td><?php echo $val->parced_domains;?></td></tr>
					<tr><th class="th-hide">Subdomains</th><td><?php echo $val->subdomains;endforeach;?></td></tr>
				</table>
			</li>
		</ul>
	</div>
</section>
<section id="portfolio" class="portfolio">
	<div class="sections-names">
		<label>FEATURED PROJECTS</label>
		<p>Our customers who have so far profited from our versatile software developing potential are from: United Kingdom, Germany, Netherlands, Switzerland, Hungary, Romania.</p>
	</div>
	<div class="div-portfolio">
		<ul class="ul-portfolio">
			<li class="li-portfolio item1"><div class="img"></div><div class="div-content"><div class="content">
				<div class="show">
					<h3>Curatenia Door to Door</h3>
					<ul class="icons-portfolio">
						<a href="http://curatenie-doortodoor.ro/" target="_blank"><li><i class="far fa-eye"></i></li>
						</a>
						<li class="plus"><i class="fas fa-search-plus"></i>
						</li>
					</ul>
				</div>
			</div>
		</div>
	</li>
	<li class="li-portfolio item2"><div class="img"></div><div class="div-content"><div class="content">
		<div class="show">
			<h3>Villanytelepi</h3>
			<ul class="icons-portfolio">
				<a href="http://villanytelep.ro" target="_blank"><li><i class="far fa-eye"></i></li>
				</a>
				<li class="plus"><i class="fas fa-search-plus"></i></li>
			</ul>
		</div></div></div>
	</li>
	<li class="li-portfolio item3"><div class="img"></div><div class="div-content"><div class="content">
		<div class="show">
			<h3>Artfoyer</h3>
			<ul class="icons-portfolio">
				<a href="http://www.artfoyer.ro/" target="_blank"><li><i class="far fa-eye"></i></li>
				</a>
				<li class="plus"><i class="fas fa-search-plus"></i></li>
			</ul>
		</div></div></div>
	</li>
	<li class="li-portfolio item4"><div class="img"></div><div class="div-content"><div class="content">
		<div class="show">
			<h3>AktivRelax</h3>
			<ul class="icons-portfolio">
				<a href="http://aktivrelax.com/" target="_blank"><li><i class="far fa-eye"></i></li>
				</a>
				<li class="plus"><i class="fas fa-search-plus"></i></li>
			</ul>
		</div></div></div>
	</li>
	<li class="li-portfolio item5"><div class="img"></div><div class="div-content"><div class="content">
		<div class="show">
			<h3>EgeszEn</h3>
			<ul class="icons-portfolio">
				<a href="http://egeszen.ro/" target="_blank"><li><i class="far fa-eye"></i></li>
				</a>
				<li class="plus"><i class="fas fa-search-plus"></i></li>
			</ul>
		</div></div></div>
	</li>
</ul>
</div>
</section>
<section id="testimonials" class="testimonials">
	<div class="sections-names">
		<label>What Our Client Say</label>
	</div>
	<div class="carousel">
		<div class="star">&#10039;</div>
		<div class="owl-carousel owl-loaded">
			<div class="owl-stage-outer">
				<div class="owl-stage">
					<div class="owl-item">"If anybody's looking for a competent, efficient, fast, reliable and a fair priced Java- and Drupal-Programmer in order to create, import and implement a database (in my case it was a XXL-sized bilingual one) into a professional business website, MILAV should be the first choice. After more than a year of frustrating search and several nasty and costly flops with a number of would-be programming "experts" I finally found an Eglish speaking programmer who was really the first one whose promisses were not made to be broken. Many thanks to Sandor and his team for an excellent approach!"
						<h2>Vladislav Herink</h2>
					</div>
					<div class="owl-item">"Jquery and mobile Jquery development. Excellent work by an accomplished developer. Would highly recommend."
						<h2>Tobye Griffiths</h2>
					</div>
				</div>
			</div>
		</div>
		<div class="slider-nav active"></div>
	</div>
</section>
<section id="about" class="about">
	<div class="img"><div class="hover"></div></div>
	<div class="sections-names">
		<label>About Us</label>
	</div>
	<div class="text">
		<p>When we design and create a webpage we want to provide you a tool that can be your key to your business in the next few years.</p>
		<p>So we craftly develop the frontend part to Your users and connect it with the backend for your content and products.</p>
		<p>As a growing company we are very flexible and highly efficient. But what can we do for you?</p>
		<p>Corporate websites, online-shops, huge portals, web- and enterprise applications, mobile Apps, full stack development, reusable web components… or in a nutshell: making your ideas come true as a digital product.</p>
	</div>
</section>
<section id="jobs" class="jobs">
	<div class="sections-names">
		<label>WE ARE HIRING YOU IF YOU ARE A...</label>
	</div>
	<div class="info-left" >
		<h4>Website Developer</h4>
		<p>Successful Candidate Profile:</p>
		<p>Education: education in IT area</p>
		<p>Required skills: PHP, HTML5, CSS3, JavaScript, communication in English</p>
		<p>Personal skills: creative attentive to details, a positive role model contributing to the team spirit, a proven ability to manage multiple projects at once</p>
		<p>Additional skills: Drupal, WordPress, Angular</p>
		<p>What we offer:</p>
		<ul>
			<li> a motivating salary package, team-buildings</li>
			<li> a pleasant work environment</li>
			<li> an office near the center of Cahul, B.P. Hașdeu Street 2</li>
			<li> a possibility to improve your personal and carrier development</li>
		</ul>
		<p>Do you think you can be a key member of our quickly-growing company? Drop us your CV and let us know why you want to be a part of our mission to do good. Our e-mail address is: <a href="mailto:jobs@milav.ro">jobs@milav.ro</a></p>
	</div>
	<div class="info-right">
		<h4>Android Developer</h4>
		<p>This is your place if:</p>
		<ul>
			<li>you are passionate about developing Android applications and optimizing the applications that already exist</li>
			<li> you have an education in IT area</li>
			<li> strong english language communication skills (B1/B2 levels)</li>
			<li> you are creative and attentive to details. Have a good team spirit</li>
			<li> a proven ability to manage multiple projects at once</li>
		</ul>
		<p>Beginners are also accepted. We offer training for learning Android programming .</p>
		<p>What we offer:</p>
		<ul>
			<li> a motivating salary package, team-buildings</li>
			<li> a pleasant work environment</li>
			<li> an office near the center of Cahul, B.P. Hașdeu Street 2</li>
			<li> a possibility to improve your personal and carrier development</li>
		</ul>
		<p>If you wish to be a part of our team, send your CV or résumé to the following e-mail address: <a href="mailto:jobs@milav.ro">jobs@milav.ro</a></p>
	</div>
</section>
<section id="contact" class="contact">
	<div class="sections-names" >
		<label>Get in Touch</label>
	</div>
	<div class="form">
		<form id="contact-form" action="" method="post">
			<div class="circle"></div>
			<div class="left-bar"></div>
			<div class="label-input" >
				<label for="name">Name</label>
				<input id="name" type="text" name="name" />
			</div>
			<div class="label-input" >
				<label for="email">Email</label>
				<input id="email" type="email" name="email" />
			</div>
			<div class="label-input">
				<label for="message">Message</label>
				<textarea id="message" name="message" rows="5" ></textarea>
			</div>
			<div class="label-input" >
				<input type="submit" value="Submit"/>
			</div>
		</form>
	</div>
</section>
<footer class="footer">
	<div class="footer-logo">
		<img src="../../public/img/logo-text.png" alt="milav logo">
	</div>
	<div class="social-media">
		<a href="https://www.facebook.com/milav.eu" target="_blank">
		<img src="../../public/img/social-facebook-circular-button.svg" alt="milav facebook">
		</a>
		<a href="https://www.linkedin.com/company/4866175/" target="_blank">
			<img src="../../public/img/social-linkedin-circular-button.svg" alt="milav linkedin">
	  </a>
	</div>
	<div class="milav-contacts">
		<div class="phone">
			<hr class="line-left">
			<img src="../../public/img/phone-call.svg" alt="milav phone">
			<hr class="line-right">
			<div class="text"><span><a href="tel:0040747191825">+40 747 191 825</a></span></div>
		</div>
		<div class="office">
			<hr class="line-left">
			<img src="../../public/img/placeholder.svg" alt="milav location">
			<hr class="line-right">
			<div class="text">
				<span>Odorheiu Secuiesc <br>
					II. Rákoczi Ferenc Street 84 <br>
					Harghita, Romania
				</span>
			</div>
		</div>
		<div class="office">
			<hr class="line-left">
			<img src="../../public/img/placeholder.svg" alt="milav location">
			<hr class="line-right">
			<div class="text">
				<span>Cahul <br>
					B.P. Hașdeu Street 2 <br>
					Rep. Moldova
				</span>
			</div>
		</div>
		<div class="mail">
			<hr class="line-left">
			<img src="../../public/img/mail.svg" alt="milav mail">
			<hr class="line-right">
			<div class="text">
				<span><a href="mailto:csatlos.sandor@milav.eu">csatlos.sandor@milav.eu</a>
				</span>
			</div>
		</div>
	</div>
	<div class="bottom">Copyright 2018 | Dermengi Ion</div>
</footer>
<div title="Back to Top" class="back-to-top">Top</div>
</body>
<link rel="stylesheet" href="../../public/css/bootstrap/bootstrap-grid.min.css">
<link rel="stylesheet" href="../../public/css/bootstrap/owl.carousel.min.css">

<script type="text/javascript" src="../../public/js/jquery/jquery.min.js"></script>
<script type="text/javascript" src="../../public/js/jquery/owl.carousel.min.js"></script>
<script type="text/javascript" src="../../public/js/custom/main.min.js"></script>
</html>